...
import { valueCommitmentData } from "@/util/Data";
import { TValueCommitment } from "@/types/globalTypes";
      ...
      <div className="   grid grid-cols-1 sm:grid-cols-2 xlm:grid-cols-3 gap-x-7 gap-y-12 ">
        {valueCommitmentData &&
          valueCommitmentData?.map((element: TValueCommitment) => (
            <Card
              title={element?.title}
              icon={<HeaderText text={element?.header} />}
            >
              <CanvasRevealEffect
                animationSpeed={3}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20">
        <div className="text-center group-hover/canvas-card:-translate-y-4 group-hover/canvas-card:opacity-0 transition duration-200 w-full  mx-auto flex items-center justify-center  absolute top-[50%] left-[50%] transform -translate-x-1/2 -translate-y-1/2 font-bold ">
          {icon}
        </div>
...
export default ValueCommitmentCard;
